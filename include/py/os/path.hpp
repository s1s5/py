...
#include "../builtins/filter.hpp"
#include "../builtins/zip.hpp"
...
std::string normcase(const std::string &path_);
inline std::string normpath(const std::string &path_) {
    ...
    if (initial_slashes &&
        ::py::startswith(path, "//") && (not ::py::startswith(path, "///"))) {
        initial_slashes = 2;
    }
...
inline std::string abspath(const std::string &path_) {
    std::string path = path_;
    if (not ::py::os::path::isabs(path)) {
        path = ::py::os::path::join(getcwd(), path);
        return ::py::os::path::normpath(path);
    }
    return ::py::os::path::normpath(path);
}

inline std::string dirname(const std::string &path) {
    return ::py::os::path::split(path).first;
}
